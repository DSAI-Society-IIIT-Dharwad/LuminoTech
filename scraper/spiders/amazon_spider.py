...
TN_PINCODES = ["600001", "641001", "620001"]
...
class AmazonBearingSpider(scrapy.Spider):
    ...
    async def parse_search_results(self, response):
        ...
        for product in products:
            ...
            price_whole = product.css(
                "span.a-price-whole::text"
            ).get("0").strip().replace(",", "")
            price_frac = product.css(
                "span.a-price-fraction::text"
            ).get("00").strip()
            mrp_raw = product.css(
        ...
        if page < 2:
            next_url = (
                ...
                f"{quote_plus(f'SKF bearing {model}')}&page={page+1}"
            )
            yield scrapy.Request(
                url=next_url,
                callback=self.parse_search_results,
    ...
    async def parse_product_page(self, response):
        ...
        # Get title from product page if missing
        if not title:
        ...
        buy_box_seller = (
            ...
            or response.css(
                "#tabular-buybox-truncate-0 .a-truncate-full::text"
            ).get("").strip()
        )
